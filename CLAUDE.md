...
| `tokens.md` | Reading the token set rather than editing it. Generated by `npm run tokens` with every reference resolved to its literal; never hand-edit, change `tokens/tokens.json` and regenerate. |
